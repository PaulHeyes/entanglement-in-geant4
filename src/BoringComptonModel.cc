 ...
 BoringComptonModel::BoringComptonModel(const G4ParticleDefinition*,
                                              const G4String& nam)
 ...
 BoringComptonModel::~BoringComptonModel()
 {  
 ...
 void BoringComptonModel::Initialise(const G4ParticleDefinition* particle,
                                        const G4DataVector& cuts)
 {
   if (verboseLevel > 3)
     G4cout << "Calling BoringComptonModel::Initialise()" << G4endl;
 ...
 G4double BoringComptonModel::ComputeCrossSectionPerAtom(
 ...
 {
   if (verboseLevel > 3)
     G4cout << "Calling ComputeCrossSectionPerAtom() of BoringComptonModel" << G4endl;
 ...
 void BoringComptonModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
                                               ...
                                               G4double)
 {
   ...
   if (verboseLevel > 3)
     G4cout << "Calling SampleSecondaries() of BoringComptonModel" << G4endl;
   ...
   do {
     ...
     if (sinThetaSqr > 1.)
       {
         G4cout
           << " -- Warning -- BoringComptonModel::SampleSecondaries "
           << "sin(theta)**2 = "
           << sinThetaSqr
           << "; set to 1"
           << G4endl;
   ...
   if(QEPhi == 0){
     analysisManager->FillH1(8, phi);
     QEPhi = phi; 
     //G4cout << "QEPhi in QEComptonModel (first gamma) = " << QEPhi << G4endl;
   }
   else{
     analysisManager->FillH1(9, phi);
     G4double delta_phi_ref = std::abs(QEPhi-phi);
     if(delta_phi_ref > pi) delta_phi_ref = std::abs(delta_phi_ref - 2*pi);
     //G4cout << "delta_phi_ref = " << delta_phi_ref << G4endl;
     analysisManager->FillH1(10, delta_phi_ref);
   }
   ...
   //gammaEnergy1 = epsilon*gammaEnergy0; // originally commmented!
 ...
 G4double BoringComptonModel::SetPhi(G4double energyRate,
                                              G4double sinSqrTh)
 {
   G4double rand1;
   G4double rand2;
 ...
 G4ThreeVector BoringComptonModel::SetPerpendicularVector(G4ThreeVector& a)
 {
   G4double dx = a.x();
   G4double dy = a.y();
   G4double dz = a.z();
 ...
 G4ThreeVector BoringComptonModel::GetRandomPolarization(G4ThreeVector& direction0)
 {
 ...
 G4ThreeVector BoringComptonModel::GetPerpendicularPolarization
 ...
 G4ThreeVector BoringComptonModel::SetNewPolarization(G4double epsilon,
                                                               G4double sinSqrTh,
                                                               G4double phi,
                                                               G4double costheta)
 {
 ...
 void BoringComptonModel::SystemOfRefChange(G4ThreeVector& direction0,
